...
app = FastAPI(
    title="OmniPass",
    description="OmniPass is a Ultimate password generator that compane arabic letter and tashkeel into password",
)
...
class PasswordGenerate:
    def __init__(self, length: int = 16, include_arabic: bool = False):
        if not (MIN_PASSWORD_LENGTH <= length <= MAX_PASSWORD_LENGTH):
            raise HTTPException(
                400,
                detail=f"Length Must be Between {MIN_PASSWORD_LENGTH}:{MAX_PASSWORD_LENGTH}",
            )
...
def getPassword(
    request: Request,
    length: int = Query(16, ge=8, le=256),
    include_arabic: bool = Query(False),
):
